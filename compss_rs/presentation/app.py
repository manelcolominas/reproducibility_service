...
def run_command(command: list[str], cwd: Path, log_dir: Path) -> int:
    ...
    console.print(f"[dim]stdout log:[/dim] {stdout_log}")
    console.print(f"[dim]stderr log:[/dim] {stderr_log}")
...
def build_summary_table(rows: list[tuple[str, str]]) -> Table:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rich MVP for COMPSs Reproducibility Service")
...
def run_app() -> int:
    ...
    console.print(
        Panel.fit(
            "COMPSs Reproducibility Service MVP",
            title="Welcome",
            subtitle="Rich CLI",
        )
    )
    ...
    console.print(f"[bold]Selected backend:[/bold] {backend}")

    crate_root = acquire_source(args.source, workspace.crate_root)
    console.print(f"[bold]Crate root:[/bold] {crate_root}")
    ...
    summary_rows = [
        ...
        ("Crate root", str(crate_root)),
        ("Backend", backend),
        ("Provenance", "yes" if args.provenance else "no"),
        ("Command", " ".join(command)),
    ]
    console.print(build_summary_table(summary_rows))

    if args.dry_run:
        console.print("[yellow]Dry run only. No execution performed.[/yellow]")
        return 0
    ...
    try:
        ...
        console.print(f"[yellow]Warning: could not move outputs into Results/: {exc}[/yellow]")

    result_text = "success" if return_code == 0 else "failure"
    console.print(
        Panel.fit(
            f"Run finished with {result_text}\nResults: {workspace.results_dir}",
            title="Final Summary",
        )
    )
    return return_code