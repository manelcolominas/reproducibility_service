...
def get_data_persistence_status(crate_path: str) -> bool:
    """
    To get data_persistence status from RO-Crate directory.
    Args:
        crate_path (str): Path for the crate directory.
    """
    dataset = os.path.join(crate_path, "dataset")
    data_persistence = False  # Default value if the dataset directory does not exist
    if os.path.exists(dataset) and os.path.isdir(dataset):
        print_colored(
            "The dataset directory exists in the crate. Data persistence wasset to True.",
            TextColor.GREEN,
        )
        data_persistence = True
    else:
        print_colored(
            "The dataset directory does not exist in the crate. Data persistence was set to False.",
            TextColor.YELLOW,
        )
    ...
    for name in os.listdir(crate_path):
        if name.endswith((".yaml", ".yml")):
            ...
            with open(yaml_file_path, "r") as file:
                ...
                data = YAML().load(file)
                if isinstance(data, dict) and "COMPSs Workflow Information" in data:
                    break
                file.close()
    if not yaml_file_path:
        # raise Exception("ro-crate-info.yaml file not found in the crate")
        print_colored(
            "ro-crate-info.yaml file not found in the crate. Creating a new one. from the template.", TextColor.RED
        )
        # Create a new ro-crate-info.yaml file from the template
        template_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "APP-REQ/ro-crate-info.yaml")

        yaml_file_path = create_yaml_from_template(template_path, crate_path) 
...
def create_yaml_from_template(template_path: str, crate_path: str) -> str:
    """
    Creates a new ro-crate-info.yaml file in the crate directory from a template.

    Args:
        template_path (str): Path to the template ro-crate-info.yaml file.
        crate_path (str): Path to the crate directory.

    Returns:
        str: Path to the newly created ro-crate-info.yaml file.
    """
    destination_path = os.path.join(crate_path, "ro-crate-info.yaml")
    shutil.copy(template_path, destination_path)
    print_colored(
        f"Created a new ro-crate-info.yaml file from the template at {destination_path}", TextColor.GREEN
    )
    ...
    if os.path.isfile(compss_submission_command_path):
        ...
        for e in crate.get_entities():
            if "#COMPSs_Workflow_Run_Crate_" in e.id:
                ...
                if desc and isinstance(desc, str) and desc.startswith(("runcompss", "enqueue_compss")):
                    new_command = desc
                    break
        if not new_command:
            for e in crate.get_entities():
                if e.type == "CreateAction" and "description" in e:
                    desc = e["description"]
                    if desc and isinstance(desc, str) and desc.startswith(("runcompss", "enqueue_compss")):
                        new_command = desc
                        break
