...
def files_verifier(
    ...
    """
    print_colored("Verifying the files in the crate", TextColor.YELLOW)
    file_verifier = []  # tuple of (file_name, file_path, included, size_ok)
    verified = True
    size_verifier = True
    temp_size = []
    temp_path = []
    ...
    # Resolve instrument to a real file path inside crate
    instrument_file = resolve_instrument_file(crate, instrument)
    if not instrument_file:
        raise ValueError(
            f"Could not resolve the instrument {instrument} to a file inside the crate."
        )

    instrument_path = os.path.join(crate_path, instrument_file)
    instrument_object = get_by_id(crate, instrument)

    # Size column default is "—" (2) when unknown/not checked
    instrument_tuple = (instrument, instrument_path, 1, 2)
    ...
    if not os.path.exists(instrument_path):
        ...
        # Verify instrument content size only when metadata has contentSize
        if instrument_object is not None and instrument_object.get("contentSize") is not None:
            expected_size = instrument_object.get("contentSize")
            actual_size = os.path.getsize(instrument_path)
    ...
    # Verify the objects/inputs
    for name, input_id in objects.items():
        # If remote dataset was fetched/mapped, skip local object with same name
        if remote_dataset_dict and name[0] in remote_dataset_dict:
            continue

        # Skip remote object references
        if isinstance(input_id, str) and input_id.startswith("http"):
            continue

        file_object = get_by_id(crate, input_id)
        if file_object is None:
            continue
        ...
        # Verify only file-like entities (or entities with contentSize)
        if "File" not in entity_type and file_object.get("contentSize") is None:
            continue

        file_path = os.path.join(crate_path, input_id)
        file_tuple = (name[0], file_path, 1, 2)
