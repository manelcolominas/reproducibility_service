...
def generate_command_line(self, sub_directory_path: str) -> list[str]:
    ...
    if os.path.isfile(compss_submission_command_path):
        ...
        crate = ROCrate(path)
        # 1. Try the COMPSs Workflow Run Crate entity
        for e in crate.get_entities():
            if "#COMPSs_Workflow_Run_Crate_" in e.id:
                ...
                if (desc and isinstance(desc, str) and desc.startswith(("runcompss", "enqueue_compss"))):
                    compss_submission_command = desc
                    break

        # 2. Fallback to any CreateAction
        if not compss_submission_command:
            for e in crate.get_entities():
        ...
        # 3. Final fallback: any entity with a valid description
        if not compss_submission_command:
            for e in crate.get_entities():
                desc = e.get("description", "")
                if ( desc and isinstance(desc, str) and desc.startswith(("runcompss", "enqueue_compss"))):
    ...
    if not compss_submission_command:
        print_colored(
            "Error: Could not generate a valid submission command starting with 'runcompss' or 'enqueue_compss'.",
            TextColor.RED,
        )
        sys.exit(1)
